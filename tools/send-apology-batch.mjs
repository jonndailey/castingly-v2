...
if (String(process.env.BATCH_OK) !== '1') {
  console.error('BATCH_OK=1 required for batch send')
  process.exit(1)
}
...
async function main() {
  ...
  for (const r of RECIPIENTS) {
    ...
    const html = emailHtml({ first: firstName(r.name), appUrl: APP_URL, password: pw })
    const out = await sendOne(r.email, subject, html)
    results.push({ email: r.email, status: out.status, id: out.id })
  }
  console.log('✅ Sent apology batch:', results)
}
